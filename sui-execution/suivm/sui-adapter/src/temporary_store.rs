...
impl<'backing> TemporaryStore<'backing> {
    ...
    fn get_object_modified_at(&self, object_id: &ObjectID) -> Option<LoadedChildObjectMetadata> {
        if self.execution_results.modified_objects.contains(object_id) {
            Some(
                self.mutable_input_refs
                    .get(object_id)
                    .map(|((version, digest), owner)| LoadedChildObjectMetadata {
                        version: *version,
                        digest: *digest,
                        owner: *owner,
                    ...
                    .or_else(|| self.loaded_child_objects.get(object_id).cloned())
                    .unwrap_or_else(|| {
                        assert!(is_system_package(*object_id));
...
type ModifiedObjectInfo<'a> = (
    ...
    Option<LoadedChildObjectMetadata>,
    Option<&'a Object>,
);
