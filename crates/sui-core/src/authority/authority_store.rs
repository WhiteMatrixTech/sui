...
use sui_types::storage::{
    get_module_by_id, BackingPackageStore, ChildObjectResolver, DeleteKind, MarkerValue, ObjectKey,
...
impl AuthorityStore {
    ...
    async fn update_objects_and_locks(
        ...
        let received_objects: Vec<_> = possible_to_receive
            .into_iter()
            .filter(|obj_ref| written.contains_key(&obj_ref.0) || deleted.contains_key(&obj_ref.0))
            .collect();
        ...
        let markers_to_place = received_objects
            ...
            .chain(deleted.iter().map(|(object_id, (version, _))| {
                (
                    (epoch_id, ObjectKey(*object_id, *version)),
