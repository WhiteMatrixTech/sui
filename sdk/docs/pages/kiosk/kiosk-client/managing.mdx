...
await kioskClient.ownedKioskTx(txb, async (kiosk, capObject) => {
        ...
        txb.transferObjects([obj], txb.pure('address_to_transfer_the_cap'));
...
kioskClient.take(txb, item.type, item.objectId, kiosk, capObject);
txb.transferObjects([obj], txb.pure('address_to_transfer_the_cap'));
...
kioskClient.convertToPersonal(tx);
```
